...
/**
 * Utility methods for retrieving actual implementing generic types from generic
 * class and fields
 *
 * @author tcaselli
 */
public class GenericsUtils {
	...
	private static List<Type> getTypeArgumentsFromInterface(final Class<?> actualType, final Class<?> baseType) {
		...
		while (!getTypeClass(type).equals(Object.class)) {
			...
			// Store type parameter (from implemented interfaces) to actual type
			// for later
			// mapping
			for (final Type superInterfaceType : allSuperInterfaces) {
				if (superInterfaceType instanceof ParameterizedType) {
					...
					final Class<?> interfaceTypeClass = getTypeClass(superInterfaceType);
					final TypeVariable<?>[] typeParameters = interfaceTypeClass.getTypeParameters();
					for (int i = 0; i < actualClassTypeArguments.length; i++) {
						resolvedTypes.put(typeParameters[i], actualClassTypeArguments[i]);
					}
				}
			...
			for (final Type genericInterface : allSuperInterfaces) {
				if (genericInterface instanceof ParameterizedType) {
					final Class<?> interfaceTypeClass = getTypeClass(genericInterface);
					if (baseType.equals(interfaceTypeClass)) {
						// Returned list
						final List<Type> typeArguments = new ArrayList<>();

						final Type[] actualTypeArguments = ((ParameterizedType) genericInterface)
								.getActualTypeArguments();
	 ...
	 *
	 * @param object
	 *            the object holding the method and on which the method must be
	 *            called
	 * @param methodName
	 *            the method name
	 * @param args
	 ...
	 *             if there is no such method in object
	 * @throws IllegalArgumentException
	 *             if arguments for the method are not correct
	...
	public static Method getMethod(final Class<?> clazz, final String methodName, final Class<?>... argsTypes) {
		...
		while (searchType != null) {
			final Method[] methods = searchType.isInterface()
		...
		if (candidates.size() == 0) {
			...
			// Here we have several candidates because of type inheritance for
			// parameters
			// For example
			...
			// For each methods and then for each parameters we will compute
			// score
			...
			if (ambiguous) {
				// Returns the one in the class the closest as possible to input
				// clazz
				...
				// // For example in spring beans , methods from parents are
				// added to proxied bean instance before methods from children.
	 ...
	 * @param recursive
	 *            whether to look for annotated method in super classes
	 *            recursively
	...
	private static TypeAndResolvedTypes getTypeAndResolvedTypes(final Class<?> childClass, final Class<?> baseClass) {
		...
		loop : while (holder.type != null) {
			...
			if (stop) {
				final Class<?> typeClass = holder.type instanceof Class
						? (Class<?>) holder.type
						: (Class<?>) ((ParameterizedType) holder.type).getRawType();
				for (final TypeVariable typeVariable : typeClass.getTypeParameters()) {
	 ...
	 * @param type
	 *            the type class
	 * @return For example for class X&lt;TYPE, TYPE2&gt; this will return
	 *         [TYPE, TYPE2]
	 */
	@SuppressWarnings("rawtypes")
	...
	/**
	 * This method retrieves {@link Class#getDeclaredMethods()} from a local
